...
* **Vault Link**: Use Linux text-processing utilities (grep/awk) to parse headers in bulk, as detailed in [`6.06_Linux_Live_Analysis`](../06_Tool_Command_Vault/6.06_Linux_Live_Analysis.md).
...
* **Vault Link**: Use native CLI decoding methods (e.g., `base64 -d`) defined in [`6.01_General_DFIR_Commands`](../06_Tool_Command_Vault/6.01_General_DFIR_Commands.md).
* **CyberChef Logic**: Use "URL Detach" and "Defang URL" recipes to sanitize the vector.

### 2. Safe Triage for Obfuscated JS/VBS Droppers

Never dump heavily obfuscated scripts (especially those found in phishing payloads or drive-by downloads) into public online unpackers (e.g., JSBeautifier, UnPacker). They often contain embedded, context-specific intel (internal domains, target emails) that shouldn't leave the environment. 

**The `console.log` / `WScript.Echo` Trick:**
Instead of trying to reverse the underlying packer algorithm (like Dean Edwards Packer), force the script to reveal its decrypted payload right before execution.

1. Copy the malicious script to an isolated local sandbox.
2. Locate the execution sink. Look for functions dynamically executing strings, most commonly:
   - `eval()`
   - `document.write()`
   - `setTimeout()`
   - `Function()`
3. Replace the execution sink with a print statement. 
   - Change `eval(function(p,a,c,k,e,d)...)` to `console.log(function(p,a,c,k,e,d)...)`
4. Run the script using a local interpreter (`node mal.js` or `cscript mal.js`).
5. The console will output the fully unpacked, weaponized payload without actually detonating it.
...
* **Vault Link**: Use [`6.01_General_DFIR_Commands`](../06_Tool_Command_Vault/6.01_General_DFIR_Commands.md) for 7z extraction (e.g., `7z x <file> -p<password>`) and SHA256 calculation (`certutil -hashfile <file> SHA256`).
...
### 1. Detection Pivot
* **Vault Link**: Apply [`6.09_Splunk_SPL_Syntaxes`](../06_Tool_Command_Vault/6.09_Splunk_SPL_Syntaxes.md) to correlate endpoint traffic with discovered malicious IPs/Domains.
