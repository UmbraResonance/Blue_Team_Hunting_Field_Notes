...
## 9. Offline Threat Hunting with Chainsaw & Sigma

> **Focus:** Rapid, at-scale EVTX parsing using Chainsaw and Sigma rule conversion.
> **Use Case:** High-speed triage of offline logs during Incident Response without a SIEM.

### Chainsaw Hunting Syntax

*Ensure the mapping file correctly matches your EVTX log structure (e.g., Sysmon vs Native Windows).*

    ```powershell
    # Basic hunt using a specific Sigma rule against an EVTX file
    .\chainsaw hunt <PATH_TO_EVTX> -s <PATH_TO_SIGMA_RULE.yml> --mapping mappings\sigma-event-logs-all.yml

    # Hunt using an entire directory of Sigma rules and output to JSON
    .\chainsaw hunt <PATH_TO_EVTX_DIR> -s <SIGMA_RULES_DIR> --mapping mappings\sigma-event-logs-all.yml --json

    # Fast keyword search (Regex/String) without Sigma (Pre-triage)
    .\chainsaw search "mimikatz" -i <PATH_TO_EVTX_DIR>
    ```

### Sigma to PowerShell Native Conversion

*Use `sigmac` (or `pySigma`) to translate Sigma logic into native `Get-WinEvent` queries for single-host analysis.*

    ```powershell
    # 1. Convert Sigma rule to PowerShell syntax
    python sigmac -t powershell <PATH_TO_SIGMA_RULE.yml>

    # 2. Execute the converted logic (Example output wrapped in Where-Object)
    Get-WinEvent -Path "<PATH_TO_EVTX>" | Where-Object { 
        ($_.ID -eq "10" -and $_.message -match "TargetImage.*.*\\lsass.exe" -and $_.message -match "GrantedAccess.*.*0x1010") 
    } | Select-Object TimeCreated, Id, RecordId, ProcessId, MachineName, Message
    ```