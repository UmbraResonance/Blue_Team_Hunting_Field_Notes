...
*Evidence: Standardizing disparate event logs into uniform fields for timeline analysis.*
...
- **Key Standardized Fields (Fact):** Maps allow EvtxECmd to convert unique `EventData` into human-readable columns such as `UserName` and `ExecutableInfo`, ensuring consistency across different OS versions and log sources.
...
*Evidence: Chronological log of file system changes (Creation, Deletion, Overwrite, Rename) mapped to specific MFT entries.*

**Parse USN Journal ($J) to CSV:**

```powershell
MFTECmd.exe -f "<PATH_TO_$EXTEND_$J>" --csv "<OUTPUT_DIRECTORY>" --csvf <OUTPUT_FILENAME.csv>
# Default path in KAPE output: C:\<KapeOut>\C\$Extend\$J
```
...
- Flag Note: `-i on` includes unassociated file entries (Important for loose binaries).
- **Forensic Value (Fact):** Amcache stores the SHA1 hash of the executable, which is critical for pivoting to threat intelligence platforms when the original file is deleted.

### Registry Transaction Log Replayer (rla.exe)

*Evidence: Ensures registry data integrity by replaying transaction logs (.LOG1/.LOG2) into the hive.*
...
- **Critical Note (Fact):** Zimmerman's tools (and RegRipper) do not automatically process Registry transaction logs. If a hive is "dirty," you must use rla.exe first to avoid missing the most recent persistence or execution traces.
...
*Evidence: Automated extraction of configuration, persistence, and activity artifacts from offline hives using specific plugins.*

**Standard Parsing Command:**

```powershell
.\rip.exe -r "<PATH_TO_HIVE_FILE>" -p <PLUGIN_NAME>
```

**Key Forensic Plugins:**

```
# System Info & Network
.\rip.exe -r "<SYSTEM_HIVE>" -p compname    # Get Hostname
.\rip.exe -r "<SYSTEM_HIVE>" -p timezone    # Get System Timezone
.\rip.exe -r "<SYSTEM_HIVE>" -p nic2        # Get NIC/IP Info

# Persistence & Autostart
.\rip.exe -r "<NTUSER_HIVE>" -p run         # User-level Auto-run entries
.\rip.exe -r "<SYSTEM_HIVE>" -p bam         # Background Activity Moderator

# User Activity
.\rip.exe -r "<NTUSER_HIVE>" -p recentdocs  # Recently accessed files
```
...
*Evidence: Constructing a unified chronological narrative by ingesting CSV outputs from MFTECmd (MFT & USN), PECmd, etc.*

**Workflow:**
1. Generate .csv files using MFTECmd or PECmd.
2. Launch Timeline Explorer (GUI).
3. Drag and drop the generated .csv files into the application.
4. Key Triage Technique: Filter by Entry Number to trace the complete lifecycle of a specific file across the USN Journal and MFT records (e.g., tracking a browser .crdownload file morphing into a malicious .exe).
