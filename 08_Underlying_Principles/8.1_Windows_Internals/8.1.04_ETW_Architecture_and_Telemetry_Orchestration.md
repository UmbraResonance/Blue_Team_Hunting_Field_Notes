...
> **Note:** This is a Restricted Provider, typically requiring PPL (Protected Process Light) status for consumption.
    ...
    - > *See also*: [8.1.29 EDR User-Mode Hooking and Unhooking Mechanics](./8.1.29_EDR_Hooking_and_Unhooking_Mechanics.md) for how ETW-TI complements user-mode hooks in modern EDR architectures, and [8.1.27 Exception Handling and TLS Callbacks](./8.1.27_Exception_Handling_and_TLS_Callbacks.md) for detection limitations when adversaries use exception-based bypass techniques that ETW does not surface.
...
## Related References
- [8.1.27 Exception Handling and TLS Callbacks](./8.1.27_Exception_Handling_and_TLS_Callbacks.md): Execution paths that bypass normal control flow and emit no ETW events.
- [8.1.28 AMSI and Script Scanning Architecture](./8.1.28_AMSI_and_Script_Scanning_Architecture.md): The `Microsoft-Windows-Antimalware-Scan-Interface` ETW provider emits per-scan events usable for post-hoc AMSI-bypass reconstruction.
- [8.1.29 EDR User-Mode Hooking and Unhooking Mechanics](./8.1.29_EDR_Hooking_and_Unhooking_Mechanics.md): Where user-mode hooks fit alongside ETW in the overall detection stack.