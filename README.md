...
| **[`03_DFIR_Playbooks`](./03_DFIR_Playbooks/)** | **Incident Response SOPs** | Standardized response workflows organized into two tracks: **`3.1_Playbooks`** (phishing analysis, network forensics) and **`3.2_Investigation_Workflows`** (memory forensics — Volatility 3 + MemProcFS for full RAM images; **[dumpex](https://github.com/bitbug0x55AA/dumpex)** for process-level `.DMP` triage). |
| **[`04_Malware_Analysis`](./04_Malware_Analysis_Cheatsheets/)**| **Malware Analysis Playbooks** | TTP-to-API behavior reference, architecture-aware PE triage, shared x86/x64 assembly foundations, x86 and x64 calling conventions, WOW64, and dynamic debugging workflows. |
